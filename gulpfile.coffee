...
### LESS ###
gulp.task 'less:vendor', ['bower'], ->
  gulp.src paths.src.less.vendor
    .pipe less paths: [
      "#{paths.dest.bower}"
    ...
    .pipe changed  paths.dest.css, {extension: '.css'}
...
gulp.task 'css', ['less:main', 'less:vendor']
...
gulp.task 'serve', ->
  ...
  gulp.watch ["#{paths.src.less.main}", "#{paths.src.self}/less/components/*.less"], ["less:main"]
  gulp.watch [paths.src.less.vendor], ["less:vendor"]
